...
> **중요**: 현재 구현에는 트랜잭션 동기화 관련 문제가 있습니다. 향후 반드시 개선이 필요합니다.

### 문제점: 락 해제 시점

현재 구현은 `@Order(1)` + try-finally 방식을 사용하여 **AOP 종료 시점에 락을 해제**합니다:
...
이 방식의 문제점:
- 서비스 메서드가 끝나는 시점에 락이 해제됨
- 트랜잭션 커밋 이후까지 락을 유지해야 하는데, 현재는 그렇지 않음
- 트랜잭션 롤백 시에도 동일한 시점에 락이 해제됨

### 개선 방향: TransactionSynchronization 사용

`TransactionSynchronizationManager`를 사용하여 **트랜잭션 커밋 후 락을 해제**하도록 변경해야 합니다:

```
[트랜잭션 시작 + 락 획득] → [비즈니스 로직] → [트랜잭션 커밋] → [락 해제]
```

개선 시 추가해야 할 사항:
1. `TransactionSynchronizationManager.registerSynchronization()`으로 콜백 등록
2. `afterCommit()` 콜백에서 락 해제
3. 트랜잭션 미활성화 상태 검증 및 예외 처리 (`LOCK_ACQUIRE_FAILED_INACTIVE_TRANSACTION`)
...
public class LockAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint, DistributedLock distributedLock) throws Throwable {
        String key = parseLockKey(...);

        // 트랜잭션 활성화 검증
        if (!TransactionSynchronizationManager.isActualTransactionActive()) {
            throw new CustomException(LOCK_ACQUIRE_FAILED_INACTIVE_TRANSACTION);
        }

        boolean acquired = lockUtil.acquireLock(key, distributedLock.timeoutSec());
        if (!acquired) {
            throw new CustomException(LOCK_ACQUIRE_FAILED);
        }

        // 트랜잭션 커밋 후 락 해제 콜백 등록
